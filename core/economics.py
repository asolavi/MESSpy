...
def NPV(file_studycase,
    ...
    economic_data: dictionary
        ...
        'carrier_name': dictionary (repeat for reach considered carrier: electricity, hydrogen, gas, ammonia)
            'purchase': [€/kWh] electricity, [€/Sm^3] gas and [€/kg] hydrogen and ammonia
            'sales': [€/kWh] electricity, [€/Sm^3] gas and [€/kg] hydrogen and ammonia
        ...
        'green_hydrogen_incentives': dictionary
            'application': bool, if True hydrogen incentives have to be considered
            'value': [€/kg] incentive value for green hydrogen production
            'n_years': number of years during which incentives apply 
        'Construction_costs': dictionary
            'application': bool, if True construction costs are included
            'engineering': fraction of total cost for engineering 
            'installation': fraction of total cost for installation 
            'excluded tech': list of technologies excluded from construction costs 
        'Additional Opex': dictionary
            'application': bool, if True additional operational expenses are included
            'personnel': [€/year] personnel costs to be applied each investment year
            
    path: str path of the input data folder
    
    name_economic: string used to customize the filename of the economic assessment results
    
    form: str file format to save results. Supported values: 'pkl'or 'csv'
    ...
    for location_name in balances:
        for carrier in balances[location_name]:
            if carrier in ['hydrogen','LP hydrogen','HP hydrogen', 'ammonia', 'oxygen','process steam','gas','water','diesel']:
                for tech in balances[location_name][carrier]:
    ...
    for location_name in tc:            # for reach location
        ...
        results[location_name]['CF_refcase'] = {  'OeM': np.zeros(economic_data['investment years']),
                                                  'Initial/Final H2 Tank level': np.zeros(economic_data['investment years']),
                                                  'Initial/Final NH3 Tank level': np.zeros(economic_data['investment years']),                                                                                                              
        ...
        for carrier in balances[location_name]:                           # for each carrier (electricity, hydrogen, ammonia, gas, heat)     
        ...
        for carrier in balances0[location_name]: # for each carrier (electricity, hydrogen, ammonia, gas, heat)
        ...
        if 'H tank' in tc[location_name]: # If final tank level is higher than initial one, the difference can be sold; purchased in the opposite case. The same process repetaed for each year
            ...
            results[location_name]['CF_studycase']['Initial/Final H2 Tank level'] = np.where(diff_values <= 0, diff_values * purchase_values, diff_values * sale_values)
            results[location_name]['CF_studycase']['Tot'] += results[location_name]['CF_studycase']['Initial/Final H2 Tank level']
            
        if 'NH3 tank' in tc[location_name]: # If final tank level is higher than initial one, the difference can be sold; purchased in the opposite case. The same process repetaed for each year
            with open('results/pkl/LOC_'+name_studycase+'.pkl', 'rb') as f: loc = pickle.load(f)
            loc = loc[location_name]['NH3 tank'][:-1] # I want a number multiple of 8760 so the lasy component is deleted
            loc = [loc[i] for i in range(0, len(loc), int(60/c.timestep))] # force hourly timestep 
            loc = np.tile(loc,years_factor)
            loc = np.reshape(loc,(-1,8760))
            diff_values = loc[:, -1] - loc[:, 0] # If final tank level is higher than initial one, the difference can be sold; purchased in the opposite case. The same process repetaed for each year
            
            if type(economic_data['ammonia']['sale']) == str:
                sale_serie = pd.read_csv(path+'/energy_price/'+economic_data['ammonia']['sale'])['0'].to_numpy()
                if len(sale_serie) < 8762: #It means that the serie must be repeated for the simulation_length selected
                    sale_serie = np.tile(sale_serie,economic_data['investment years'])
                sale_serie = np.reshape(sale_serie,(-1,8760))
                sale_values = np.mean(sale_serie, axis=1)  #€/kg
            else:
                sale_values = np.tile(economic_data['ammonia']['sale'],economic_data['investment years'])  #€/kg

            if type(economic_data['ammonia']['purchase']) == str:
                purchase_serie = pd.read_csv(path+'/energy_price/'+economic_data['ammonia']['purchase'])['0'].to_numpy()
                if len(purchase_serie) < 8762: #It means that the serie must be repeated for the simulation_length selected
                    purchase_serie = np.tile(purchase_serie,economic_data['investment years'])
                purchase_serie = np.reshape(purchase_serie,(-1,8760))
                purchase_values = np.mean(purchase_serie, axis=1)  #€/kg
            else:
                purchase_values = np.tile(economic_data['ammonia']['purchase'],economic_data['investment years'])  #€/kg

            results[location_name]['CF_studycase']['Initial/Final NH3 Tank level'] = np.where(diff_values <= 0, diff_values * purchase_values, diff_values * sale_values)
            results[location_name]['CF_studycase']['Tot'] += results[location_name]['CF_studycase']['Initial/Final NH3 Tank level']
        ...
        if 'H tank' in tc0[location_name]:
            ...
            results[location_name]['CF_refcase']['Initial/Final H2 Tank level'] = np.where(diff_values <= 0, diff_values * purchase_values, diff_values * sale_values)
            results[location_name]['CF_refcase']['Tot'] += results[location_name]['CF_refcase']['Initial/Final H2 Tank level']
            
        if 'NH3 tank' in tc0[location_name]:
            with open('results/pkl/LOC_'+name_refcase+'.pkl', 'rb') as f: loc = pickle.load(f)
            loc = loc[location_name]['NH3 tank'][:-1] # I want a number multiple of 8760 so the lasy component is deleted
            loc = [loc[i] for i in range(0, len(loc), int(60/c.timestep))] # force hourly timestep 
            loc = np.tile(loc,years_factor)
            ...
            results[location_name]['CF_refcase']['Initial/Final NH3 Tank level'] = np.where(diff_values <= 0, diff_values * purchase_values, diff_values * sale_values)
            results[location_name]['CF_refcase']['Tot'] += results[location_name]['CF_refcase']['Initial/Final NH3 Tank level']
        ...
        # Adding personnel cost for study case and reference case
        if economic_data['Additional Opex']['application']:
            personnel = - economic_data['Additional Opex']['personnel']
            results[location_name]['CF_studycase']['personnel'] = np.tile(personnel, economic_data['investment years'])
            results[location_name]['CF_studycase']['Tot'] += results[location_name]['CF_studycase']['personnel']
        ...
        results[location_name]['CF']['Initial/Final H2 Tank level'] += results[location_name]['CF_studycase']['Initial/Final H2 Tank level'] - results[location_name]['CF_refcase']['Initial/Final H2 Tank level']
        results[location_name]['CF']['Initial/Final NH3 Tank level'] += results[location_name]['CF_studycase']['Initial/Final NH3 Tank level'] - results[location_name]['CF_refcase']['Initial/Final NH3 Tank level']
        ...
        # Must additional costs for installation, engineering and civil works be taken into account?
        if economic_data['Construction_costs']['application']:
            if economic_data['Construction_costs']['excluded tech']: # for this technologies the installation and engineering costs are already included in the price 
               I = results[location_name]['I0_studycase']['Tot']    
               for tech in economic_data['Construction_costs']['excluded tech']:
                    I = I - results[location_name]['I0_studycase'][tech]
               results[location_name]['I0_studycase']['engineering'] = economic_data['Construction_costs']['engineering']*I
               results[location_name]['I0_studycase']['installation'] = economic_data['Construction_costs']['installation']*I
            else:
                results[location_name]['I0_studycase']['engineering'] = economic_data['Construction_costs']['engineering']*(results[location_name]['I0_studycase']['Tot'])
                results[location_name]['I0_studycase']['installation'] = economic_data['Construction_costs']['installation']*(results[location_name]['I0_studycase']['Tot'])

            results[location_name]['I0']['engineering'] = results[location_name]['I0_studycase']['engineering']
            results[location_name]['I0']['installation'] = results[location_name]['I0_studycase']['installation']
            results[location_name]['I0']['Tot'] += results[location_name]['I0']['installation'] + results[location_name]['I0']['engineering'] 
...
def LCOH (location_name,
    ...
    # Adding the calculation for personnel and land rent contributions
    if economic_data['Additional Opex']['application']:
        personnel = economic_data['Additional Opex']['personnel']
        results_pp[location_name]['CF']['personnel'] = np.tile(- personnel, economic_data['investment years'])
        
        lcoh[location_name]['Opex']['personnel']  = np.repeat(personnel,economic_data['investment years'])
    
        results_pp[location_name]['CF']['Tot'] += - results_pp[location_name]['CF']['personnel']
    ...
    if economic_data['Construction_costs']['application']:
        if economic_data['Construction_costs']['excluded tech']: # for this technologies the installation and engineering costs are already included in the price 
           I = results_pp[location_name]['I0']['Tot']    
           for tech in economic_data['Construction_costs']['excluded tech']:
                I = I - results_pp[location_name]['I0'][tech]
           results_pp[location_name]['I0']['engineering'] = economic_data['Construction_costs']['engineering']*I
           results_pp[location_name]['I0']['installation'] = economic_data['Construction_costs']['installation']*I
        else:
            results_pp[location_name]['I0']['engineering'] = economic_data['Construction_costs']['engineering']*(results_pp[location_name]['I0']['Tot'])
            results_pp[location_name]['I0']['installation'] = economic_data['Construction_costs']['installation']*(results_pp[location_name]['I0']['Tot'])
        results_pp[location_name]['I0']['Tot'] = results_pp[location_name]['I0']['Tot'] + results_pp[location_name]['I0']['installation'] + results_pp[location_name]['I0']['engineering'] 
     
        lcoh[location_name]['Capex']['engineering'] = np.concatenate(([results_pp[location_name]['I0']['engineering']], np.full(economic_data['investment years'], 0))) # Capex array
        lcoh[location_name]['Capex']['installation'] = np.concatenate(([results_pp[location_name]['I0']['installation']], np.full(economic_data['investment years'], 0))) # Capex array
...
def LCOE (location_name,
    ...
    'REC': dictionary REC economic parameters definition
        'collective self consumption incentives': [€/kWh]
        'incentives redistribution': 0-100 how the incentives are redistributed between prosumers, consumers and REC manger
    'carrier_name': dictionary (repeat for reach considered carrier: electricity, hydrogen, ammonia, gas)
        'purchase': [€/kWh] electricity, [€/Sm^3] gas and [€/kg] hydrogen and ammonia
        'sales': [€/kWh] electricity, [€/Sm^3] gas and [€/kg] hydrogen and ammonia
    'interest rate': 0-1 [rate/year]
    'inflation rate': -1-1 [rate/year] cost evolution of each carrier
    ...
    techs_of_interest = ['PV', 'wind', 'fuel cell', 'CCGT']
    present_technologies = []

    for tech_name in structure[location_name]:
        if tech_name in techs_of_interest:
    ...
    if 'wind' in structure[location_name] or 'PV' in structure[location_name] or 'fuel cell' in structure[location_name] or 'thermoelectric' in structure[location_name]:
        pass
    else:
    ...
    if economic_data['Additional Opex']['application']:
        ...
        lcoe[location_name]['Opex']['personnel']  = np.repeat(personnel,economic_data['investment years'])
    
        results_pp[location_name]['CF']['Tot'] += - results_pp[location_name]['CF']['personnel']
        

    # energy sold and purchased in study case 
    for carrier in production[location_name]:                           # for each carrier (electricity, hydrogen, ammonia, gas, heat)
    ...
    # Building Cash Flow final array while changing sign to revenews and expenditures as needed in LCOE formula
    for carrier in results_pp[location_name]['CF']['Purchase']:
        results_pp[location_name]['CF']['Tot'] -= results_pp[location_name]['CF']['Purchase'][carrier]
        
    if revenues:        # if revenues have to be included in LCOE calculation
        for carrier in results_pp[location_name]['CF']['Sale']:
            if carrier in revenues:
                results_pp[location_name]['CF']['Tot'] -= results_pp[location_name]['CF']['Sale'][carrier]
        # lcoe dictionary
    ...
    produced_electricity_pv, produced_electricity_wind, produced_electricity_fuelcell, produced_electricity_CCGT = [np.zeros(c.simulation_years),np.zeros(c.simulation_years),np.zeros(c.simulation_years),np.zeros(c.simulation_years)]

    if 'PV' in production[location_name]['electricity']:
        produced_electricity_pv = np.array([0] + [(sum(production[location_name]['electricity']['PV']['Tot']))/c.simulation_years]*economic_data['investment years'])  # [kWh/y] - No electricity produced in period 0
    if 'wind' in production[location_name]['electricity']:
        produced_electricity_wind = np.array([0] + [(sum(production[location_name]['electricity']['wind']['Tot']))/c.simulation_years]*economic_data['investment years'])  # [kWh/y] - No electricity produced in period 0
    if 'fuel cell' in production[location_name]['electricity']:
        produced_electricity_fuelcell = np.array([0] + [(sum(production[location_name]['electricity']['fuel cell']['Tot']))/c.simulation_years]*economic_data['investment years'])  # [kWh/y] - No electricity produced in period 0
    if 'CCGT' in production[location_name]['electricity']:
        produced_electricity_CCGT = np.array([0] + [(sum(production[location_name]['electricity']['CCGT']['Tot']))/c.simulation_years]*economic_data['investment years'])  # [kWh/y] - No electricity produced in period 0

    if 'electricity demand' in structure[location_name]:
        produced_electricity = np.array([0] + [(sum(consumption[location_name]['electricity']['electricity demand']['Tot']))/c.simulation_years]*economic_data['investment years'])  # [kWh/y] - No electricity produced in period 0
    else:
        produced_electricity = produced_electricity_CCGT+produced_electricity_pv+produced_electricity_wind+produced_electricity_fuelcell
    ...
    if economic_data['Construction_costs']['application']:
        ...
        lcoe[location_name]['Capex']['engineering'] = np.concatenate(([results_pp[location_name]['I0']['engineering']], np.full(economic_data['investment years'], 0))) # Capex array
        lcoe[location_name]['Capex']['installation'] = np.concatenate(([results_pp[location_name]['I0']['installation']], np.full(economic_data['investment years'], 0))) # Capex array
    ...
    results_pp[location_name]['LCOE'] = {'Value [€/kWh]'          : LCOE,
                                         'Discounted Expenditures' : num,
                                         'Discounted Production'   : den}
    ...
    df1 = df1.round(4)
    df1 = df1.drop(columns=df1.columns[(df1.loc['Capex'] == 0) & (df1.loc['Opex'] == 0)])    # removing elements not contributing to final LCOE value
    ...
    if plot == True:
        ...
        if isinstance(revenues, list) and 'compensation fee' in revenues:
            if 'PV' in structure[location_name]:
                df.loc['Opex']['Total'] -= df.loc['Opex']['PV Compensation']
                df = df.drop(columns = 'PV Compensation')
                df1.loc['Opex']['PV Compensation'] = -df1.loc['Opex']['PV Compensation']
            elif 'wind' in structure[location_name]:
                df.loc['Opex']['Total'] -= df.loc['Opex']['Wind Compensation']
                df = df.drop(columns = 'Wind Compensation')    
                df1.loc['Opex']['Wind Compensation'] = -df1.loc['Opex']['Wind Compensation']
    ...
    # saving results_pp in results/pkl
    with open("results/pkl/LCOE_assessment.pkl", 'wb') as f:  pickle.dump(results_pp,f)
    ...
    df.to_csv('results/csv/LCOE_contributions.csv',index=True,sep=';',decimal=',')
...
def LCOS (location_name,
    ...
    economic_data: dictionary
        'REC': dictionary REC economic parameters definition
            'collective self consumption incentives': [€/kWh]
            'incentives redistribution': 0-100 how the incentives are redistributed between prosumers, consumers and REC manger
        'carrier_name': dictionary (repeat for reach considered carrier: electricity, hydrogen, ammmonia, gas)
            'purchase': [€/kWh] electricity, [€/Sm^3] gas and [€/kg] hydrogen and ammonia
            'sales': [€/kWh] electricity, [€/Sm^3] gas and [€/kg] hydrogen and ammonia
        'interest rate': 0-1 [rate/year]
        'inflation rate': -1-1 [rate/year] cost evolution of each carrier
    ...
    storage_type: dict of storage systems to their corresponding output technology (e.g. 'NH3 tank' → 'CCGT', 'battery' → 'battery')
        
    techs_of_interest: list of technologies to be included for the calculation of the LCOS 
    
    revenues: list/tuple of str/bool defining if generated revenues from excess energy streams have to be included in the calculation. In case the carrier name(s) is/are specified. Default = False
    
    refund: boolean value defining refund cash flows have to be included in the calculation. Default = False
                        
    Output: [€/kgH2] or [€/kgNH3] or [€/kWh] float value of LCOS for the considered configuration

    """  
    

    if 'H tank' in structure[location_name] or 'NH3 tank' in structure[location_name] or 'battery' in structure[location_name]:
        pass
    else:
        raise ValueError("Error: No storage system is employed in the system. Include it to calculate the LCOS.")
    ...
    # balances are now in the form of yearly arrays of 8760 values. Converting energy carriers and material streams expressed as mass flow rates into hourly values necessary for economic analysis purposes. kg/s to kg/h and Sm^3/s to Sm^3/h
    for carrier in consumption[location_name]:
        if carrier in ['hydrogen','LP hydrogen','HP hydrogen','ammonia','oxygen','process steam','gas','water']:
            for tech_name in consumption[location_name][carrier]:
                for tech_name1 in consumption[location_name][carrier][tech_name]:
                    consumption[location_name][carrier][tech_name][tech_name1] = consumption[location_name][carrier][tech_name][tech_name1]*3600
    ...
    results_pp  = {}    # dictionary initialising global lcos results of each location
    lcos        = {}    # dictionary initialising specific lcos results of each location
        
    results_pp[location_name]   = {}    # dictionary initialising economic results_pp
    lcos[location_name]         = {}    # dictionary initialising economic results_pp
    ...
    lcos[location_name]['Capex']    = {}
    lcos[location_name]['Opex']     = {}
    ...
    for tech_name in tc[location_name]:              # considering each technology in the location
        ...
        if tech_name in techs_of_interest:    
            ...
            lcos[location_name]['Capex'][tech_name] = np.concatenate(([tc[location_name][tech_name]['total cost']], np.full(economic_data['investment years'], 0))) # Capex array
            lcos[location_name]['Opex'][tech_name]  = np.repeat(tc[location_name][tech_name]['OeM'],economic_data['investment years']) # Opex array  
            ...
            if tc[location_name][tech_name]['replacement']['years'] == "ageing": # if replacement year is calculated according to ageing
                with open('results/pkl/ageing_'+name_studycase+'.pkl', 'rb') as f:
                    ...
                    for a in age:
                        rep_time = int(a/8760)
                        results_pp[location_name]['CF']['OeM'][rep_time] += - results_pp[location_name]['I0'][tech_name] * tc[location_name][tech_name]['replacement']['rate']/100 # subtract technology replacement to location Cash Flow
                        lcos[location_name]['Capex'][tech_name] += lcos[location_name]['Capex'][tech_name] * tc[location_name][tech_name]['replacement']['rate']/100 # subtract technology replacement to location Cash Flow
            else: # if replacement time is given
                rep_time = tc[location_name][tech_name]['replacement']['years']
                while rep_time < economic_data['investment years']: # if tech_name replacement happens before the end of the simulation
                    results_pp[location_name]['CF']['OeM'][rep_time-1] += - np.float64(results_pp[location_name]['I0'][tech_name])*np.float64(tc[location_name][tech_name]['replacement']['rate'])/np.float64(100) # subtract technology replacement to location Cash Flow. -1 subtracted to index because the array is shifted of 1 position during lcos calculation -> CF[1:]  = results_pp[location_name]['CF']['Tot'].copy()    # [€] Cash Flows
                    lcos[location_name]['Capex'][tech_name][rep_time] += np.float64(lcos[location_name]['Capex'][tech_name][0])*np.float64(tc[location_name][tech_name]['replacement']['rate'])/np.float64(100) # subtract technology replacement to location Cash Flow. np.float type added to avoid overflow problems 
                    rep_time += tc[location_name][tech_name]['replacement']['years']
            
            if refund: # if refund has to be included in lcos calculation
                    
                if tc[location_name][tech_name]['refund']['years'] == 0 and tc[location_name][tech_name]['refund']['rate'] == 0:  # no refunds considered
                    pass
                elif tc[location_name][tech_name]['refund']['years'] == 0 and tc[location_name][tech_name]['refund']['rate'] != 0:
                    results_pp[location_name]['I0'][tech_name] = results_pp[location_name]['I0'][tech_name]*((100-tc[location_name][tech_name]['refund']['rate'])/100)
                    lcos[location_name]['Capex'][tech_name] = lcos[location_name]['Capex'][tech_name]*((100-tc[location_name][tech_name]['refund']['rate'])/100)
                else:
                    yearly_refund = results_pp[location_name]['I0'][tech_name]*tc[location_name][tech_name]['refund']['rate']/100 / tc[location_name][tech_name]['refund']['years'] # yearly refund [€]
                    refunds = np.zeros(economic_data['investment years']) # array initialise
                    refunds[:min(economic_data['investment years'],tc[location_name][tech_name]['refund']['years'])] = yearly_refund # array repeat yearly refund 
                    results_pp[location_name]['CF']['Refund'] += refunds # add refund to Cash Flow
                    lcos[location_name]['Opex'][tech_name] -= refunds # add refund to Opex expenditures
    ...
    for carrier in economic_data['inflation rate']:
        ...
        for k in lcos[location_name]['Opex']: 
            if carrier in k:
                for y in range(economic_data['investment years']):
                    lcos[location_name]['Opex'][k][y] = lcos[location_name]['Opex'][k][y]*(1+f)**y
    ...
    # Must additional costs for installation, engineering  be taken into account?
    if economic_data['Construction_costs']['application']:
        if economic_data['Construction_costs']['excluded tech']: # for this technologies the installation and engineering costs are already included in the price 
           I = results_pp[location_name]['I0']['Tot']    
           for tech in economic_data['Construction_costs']['excluded tech']:
               if tech in techs_of_interest:
                   I = I - results_pp[location_name]['I0'][tech]
           results_pp[location_name]['I0']['engineering'] = economic_data['Construction_costs']['engineering']*I
           results_pp[location_name]['I0']['installation'] = economic_data['Construction_costs']['installation']*I
        else:
        ...
        lcos[location_name]['Capex']['engineering'] = np.concatenate(([results_pp[location_name]['I0']['engineering']], np.full(economic_data['investment years'], 0))) # Capex array
        lcos[location_name]['Capex']['installation'] = np.concatenate(([results_pp[location_name]['I0']['installation']], np.full(economic_data['investment years'], 0))) # Capex array
                    
    for key in lcos[location_name]['Opex']:
        lcos[location_name]['Opex'][key] = np.insert(lcos[location_name]['Opex'][key], 0, 0) # shifting Opex value to year 1 while only Capex is considered in year 0.
        
    lcos[location_name]['Capex']['Total'] = sum(lcos[location_name]['Capex'].values())  # Adding Capex total values to dictionary
    lcos[location_name]['Opex']['Total'] = sum(lcos[location_name]['Opex'].values())    # Adding Opex total values to dictionary
   
    # LCOS calculation
    output = [0] * (economic_data['investment years'] + 1)
    
    tank_types = ['H tank', 'NH3 tank']
    tanks_present = [stype for stype in storage_type if stype in tank_types]
    
    for stype, output_tech in storage_type.items():
        if stype == 'battery' and output_tech != 'battery':
            raise ValueError("The only admissible output tech for 'battery' is 'battery' itself")
        if stype in tank_types:
            if len(storage_type) > 1 and output_tech == stype:
                raise ValueError(
                    f"Inconsistent output technology for '{stype}': {output_tech}. "
                    "A tank can output itself only if it is the only storage type. "
                    "When multiple storage types are present, a tank must output electricity (e.g., CCGT).")
            
    if 'H tank' in storage_type:
        output_tech = storage_type['H tank']
        if output_tech == 'H tank':
            contrib = [0] + [(sum(production[location_name]['hydrogen'][output_tech]['Tot'])) / c.simulation_years] * economic_data['investment years']
        elif output_tech == 'CCGT':
            contrib = [0] + [(sum(production[location_name]['electricity'][output_tech]['electricity demand'])) / c.simulation_years] * economic_data['investment years']
        output = [a + b for a, b in zip(output, contrib)]
    
    if 'NH3 tank' in storage_type:
        output_tech = storage_type['NH3 tank']
        if output_tech == 'NH3 tank':
            contrib = [0] + [(sum(production[location_name]['ammonia'][output_tech]['Tot'])) / c.simulation_years] * economic_data['investment years']
        elif output_tech == 'CCGT':
            contrib = [0] + [(sum(production[location_name]['electricity'][output_tech]['electricity demand'])) / c.simulation_years] * economic_data['investment years']
        output = [a + b for a, b in zip(output, contrib)]
    
    if 'battery' in storage_type:
        output_tech = storage_type['battery']
        contrib = [0] + [(sum(production[location_name]['electricity'][output_tech]['Tot'])) / c.simulation_years] * economic_data['investment years']
        output = [a + b for a, b in zip(output, contrib)]
    ...
    if economic_data['decommissioning'] > 0:
        CF = np.append(CF, I0*economic_data['decommissioning'])
        output.append(0)
        lcos[location_name]['Capex']['Total'] = np.append(lcos[location_name]['Capex']['Total'], I0*economic_data['decommissioning'])
    ...
    for i in range(len(CF)):
        ...
        num.append((CF[i])*(1/(1+r)**i))
        den.append(output[i]*(1/(1+r)**i))
    
    LCOS = round(sum(num)/sum(den),3)                      
    
    if 'H tank' in storage_type: 
        if storage_type['H tank'] == 'H tank':
            UM = '[€/kgH2]'
            unit = "[€/kgH$_\mathregular{2}$]"
    if 'NH3 tank' in storage_type:
        if storage_type['NH3 tank'] == 'NH3 tank':
            UM = '[€/kgNH3]'
            unit = "[€/kgNH$_\mathregular{3}$]"
    if 'battery' in storage_type or ('H tank' in storage_type and storage_type['H tank'] == 'CCGT') or ('NH3 tank' in storage_type and storage_type['NH3 tank'] == 'CCGT'):
        UM = '[€/kWh]'
        unit = " [€/kWh]"
        
    results_pp[location_name]['LCOS'] = {f'Value {UM}'          : LCOS,
                                         'Discounted Expenditures' : num,
                                         'Discounted Production'   : den}
            
    for key in lcos[location_name]['Opex']:
        for i in range(len(lcos[location_name]['Opex'][key])):
            lcos[location_name]['Opex'][key][i] = lcos[location_name]['Opex'][key][i]*(1/(1+r)**i)
    
    for key in lcos[location_name]['Capex']:
        for i in range(len(lcos[location_name]['Capex'][key])):
            lcos[location_name]['Capex'][key][i] = lcos[location_name]['Capex'][key][i]*(1/(1+r)**i)
    
    # specific costs associated with each technology. Data handling.
    capex   = {}
    opex    = {}
    
    for key, value in lcos[location_name]['Capex'].items():
        capex[key] = np.sum(value)
    for key, value in lcos[location_name]['Opex'].items():
        opex[key] = np.sum(value)
    ...
    df1 = df1/hydrogen_prod
    df1 = df1.round(4)
    df1 = df1.drop(columns=df1.columns[(df1.loc['Capex'] == 0) & (df1.loc['Opex'] == 0)])    # removing elements not contributing to final lcos value
    ...
    if plot == True:
        ...
        df = df.drop(columns=df.columns[(df.loc['Capex'] == 0) & (df.loc['Opex'] == 0)])    # removing elements not contributing to final lcos value
        df = df.drop(columns=[col for col in df.columns if 'sold' in col])
        df = df/hydrogen_prod
        df = df.round(4)
  
        colors = ['#0e4d92','#2380b2', '#c8e0a1', '#ebd279', '#e18b4f', '#ff0000', '#00ff00', '#0000ff']
        colors1 = ['#7fcdbb', '#edf8b1']       
        ...
        repeated_labels = [label for label in labels_outer if labels_outer.count(label) > 0]
        ...
        column_labels = [label + unit for label in df.index]
        table = plt.table(cellText=df1.T.values,  
                  colLabels=column_labels,    
                  rowLabels=df1.columns,  
                  cellLoc='center',
        ...
        plt.suptitle(f'LCOS = {LCOS} {unit}', y=0.05, fontsize=18, fontweight='bold', ha='center')
        plt.subplots_adjust(bottom=0.18)    
    ...
    # saving results_pp in results/pkl
    with open(f"results/pkl/lcos_assessment.pkl", 'wb') as f:  pickle.dump(results_pp,f)
    ...
    df_.index = pd.MultiIndex.from_tuples(df_.index, names=['level_1', 'level_2', 'level_3', 'values'])
    df_ = df_.transpose()
    df_.to_csv('results/csv/lcos_assessment.csv',index=False,sep=';',decimal=',')
    
    if print_ == True:
        print("\nThe LCOS calculated for the considered scenario for "+location_name+" location results in "+str(LCOS)+" "+UM)
    
    return(LCOS) 


def TAC(file_studycase,
        file_refcase,
        name_studycase,
        name_refcase,
        economic_data,
    ...
    results_TAC = {}                        # dictionary initialise economic results of each locations
    
    peakP = sum(location_name.get('PV', {}).get('peakP',0) for location_name in studycase.values()) + sum(location_name.get('wind', {}).get('Npower',0)for location_name in studycase.values())/1000  #calculate total nominal power of REC
    
    for location_name in tc:            # for each location
        
        results_TAC[location_name] = {}     # dictionary initialise economic results
       
        # initialise cash flow:
     
        results_TAC[location_name]['CF_studycase'] =  {'OeM': np.zeros(economic_data['investment years']),
                                                  'Initial/Final H2 Tank level': np.zeros(economic_data['investment years']), 
                                                  'Initial/Final NH3 Tank level': np.zeros(economic_data['investment years']),                                                                                            
                                                  'green_hydrogen_incentives': np.zeros(economic_data['investment years']),                                                                                                                           
                                                  'Purchase': {},
                                                  ...
                                                  'CSC': np.zeros(economic_data['investment years']),
                                                  'Tot': np.zeros(economic_data['investment years'])}          
        results_TAC[location_name]['CF'] =  {'OeM': np.zeros(economic_data['investment years']),
                                                  'Initial/Final H2 Tank level': np.zeros(economic_data['investment years']), 
                                                  'Initial/Final NH3 Tank level': np.zeros(economic_data['investment years']),                                                                                            
                                                  'green_hydrogen_incentives': np.zeros(economic_data['investment years']),                                                                                                                           
                                                  'Purchase': {},
        ...
        results_TAC[location_name]['I0_refcase']    = {} # MC initialise investment refcase
        results_TAC[location_name]['I0_studycase']  = {} # MC initialise investment studycase                                                                                                                                                                    
        results_TAC[location_name]['I0']            = {} # initialise initial investment    
        results_TAC[location_name]['AF']            = {} # initialise annuity factor
        
        for tech_name in tc[location_name]:         # considering each technology in the location
        
            results_TAC[location_name]['I0_studycase'][tech_name] = tc[location_name][tech_name]['total cost'] # MC I0 studycase
            results_TAC[location_name]['CF_studycase']['OeM'][:] += - tc[location_name][tech_name]['OeM'] # OeM
            ...
            if tc[location_name][tech_name]['replacement']['years'] == "ageing": # if replacement year is calculated according to ageing
                with open('results/pkl/ageing_'+name_studycase+'.pkl', 'rb') as f:
                    ...
                    for a in age:
                        rep_time = int(a/8760)
                        results_TAC[location_name]['CF_studycase']['OeM'][rep_time] += - results_TAC[location_name]['I0_studycase'][tech_name] * tc[location_name][tech_name]['replacement']['rate']/100 # subtract technology replacement to location Cash Flow
            else: # if replacement time is given
                rep_time = tc[location_name][tech_name]['replacement']['years']
                while rep_time < economic_data['investment years']: # if tech_name replacement happens before the end of the simulation
                    results_TAC[location_name]['CF_studycase']['OeM'][rep_time] += - results_TAC[location_name]['I0_studycase'][tech_name] * tc[location_name][tech_name]['replacement']['rate']/100 # subtract technology replacement to location Cash Flow
            ...
            # refund
            if tc[location_name][tech_name]['refund']['years'] == 0:
                results_TAC[location_name]['I0_studycase'][tech_name] = results_TAC[location_name]['I0_studycase'][tech_name]*(100-tc[location_name][tech_name]['refund']['rate'])/100
            else:
                yearly_refund = results_TAC[location_name]['I0_studycase'][tech_name]*tc[location_name][tech_name]['refund']['rate']/100 / tc[location_name][tech_name]['refund']['years'] # yearly refund [€]
                refunds = np.zeros(economic_data['investment years']) # array initialise
                refunds[:min(economic_data['investment years'],tc[location_name][tech_name]['refund']['years'])] = yearly_refund # array repet yearly refond 
                results_TAC[location_name]['CF_studycase']['Refund'] += refunds # add refund to Cash Flow
            
        for tech_name in tc0[location_name]:
            
            results_TAC[location_name]['I0_refcase'][tech_name] = tc0[location_name][tech_name]['total cost'] #MC IO refcase  
        ...
        for carrier in balances[location_name]:                           # for each carrier (electricity, hydrogen, gas, heat)     
            if carrier+' grid' in balances[location_name][carrier]:  
                ...
                sold = np.tile(sold,years_factor)
                sold = np.reshape(sold,(-1,8760))
                results_TAC[location_name]['CF_studycase']['Sale'][carrier]   = - sold.sum(axis=1,where=sold<0)
                results_TAC[location_name]['CF']['Sale'][carrier] = np.zeros(economic_data['investment years'])
                ...
                purchase = np.tile(purchase,years_factor)
                purchase = np.reshape(purchase,(-1,8760))
                results_TAC[location_name]['CF_studycase']['Purchase'][carrier]   = - purchase.sum(axis=1,where=purchase>0)
                results_TAC[location_name]['CF']['Purchase'][carrier] = np.zeros(economic_data['investment years']) 
            ...
            if carrier == 'electricity':    # Electricity purchased even by PV and wind if not owned
                if 'wind' in studycase[location_name] and studycase[location_name]['wind']['owned'] == False:
                    ...
                    purchase = np.tile(purchase,years_factor)
                    purchase = np.reshape(purchase,(-1,8760))
                    results_TAC[location_name]['CF_studycase']['Purchase'][carrier] += - purchase.sum(axis=1)
        ...
        if 'REC' in economic_data:
            ...
            inc_pro = np.tile(inc_pro,years_factor)
            inc_pro = np.reshape(inc_pro,(-1,8760))    
            results_TAC[location_name]['CF_studycase']['CSC'] += inc_pro.sum(axis=1,where=inc_pro>0) 
            
            inc_con = csc * economic_data['REC']['incentives redistribution']['consumers']/100 * csc_inc
            inc_con= np.tile(inc_con,years_factor)
            inc_con = np.reshape(inc_con,(-1,8760))
            results_TAC[location_name]['CF_studycase']['CSC'] += inc_con.sum(axis=1,where=inc_con>0)   
        ...
        for carrier in economic_data['inflation rate']:
            ...
            if carrier in results_TAC[location_name]['CF_studycase']['Purchase']:
                for y in range(economic_data['investment years']):
                    results_TAC[location_name]['CF_studycase']['Purchase'][carrier][y] = results_TAC[location_name]['CF_studycase']['Purchase'][carrier][y]*(1+f)**y
                    
            if carrier in results_TAC[location_name]['CF_studycase']['Sale']:
                for y in range(economic_data['investment years']):
                    results_TAC[location_name]['CF_studycase']['Sale'][carrier][y] = results_TAC[location_name]['CF_studycase']['Sale'][carrier][y]*(1+f)**y
        ...
        if 'H tank' in tc[location_name]: # If final tank level is higher than initial one, the difference can be sold; purchased in the opposite case. The same process repetaed for each year
            ...
            results_TAC[location_name]['CF_studycase']['Initial/Final H2 Tank level'] = np.where(diff_values <= 0, diff_values * purchase_values, diff_values * sale_values)
            results_TAC[location_name]['CF_studycase']['Tot'] += results_TAC[location_name]['CF_studycase']['Initial/Final H2 Tank level']
        ...
        if 'NH3 tank' in tc[location_name]: # If final tank level is higher than initial one, the difference can be sold; purchased in the opposite case. The same process repetaed for each year
            ...
            results_TAC[location_name]['CF_studycase']['Initial/Final NH3 Tank level'] = np.where(diff_values <= 0, diff_values * purchase_values, diff_values * sale_values)
            results_TAC[location_name]['CF_studycase']['Tot'] += results_TAC[location_name]['CF_studycase']['Initial/Final NH3 Tank level']
        ...
        if 'electrolyzer' in tc[location_name] and economic_data['green_hydrogen_incentives']['application'] == True: # If hydrogen incentives have to be considered
            ...
            h2_produced = np.reshape(h2_produced,(-1,8760))
            results_TAC[location_name]['CF_studycase']['green_hydrogen_incentives'][0:n_years_incentives] = np.sum(h2_produced, axis=1)*incentive_value
            results_TAC[location_name]['CF_studycase']['Tot'] += results_TAC[location_name]['CF_studycase']['green_hydrogen_incentives']
        
        # Adding personnel cost for study case and reference case
        if economic_data['Additional Opex']['application']:
            personnel = - economic_data['Additional Opex']['personnel']
            results_TAC[location_name]['CF_studycase']['personnel'] = np.tile(personnel, economic_data['investment years'])
            results_TAC[location_name]['CF_studycase']['Tot'] += results_TAC[location_name]['CF_studycase']['personnel']
            results_TAC[location_name]['CF']['personnel'] = np.tile(personnel, economic_data['investment years'])
            results_TAC[location_name]['CF']['Tot'] += results_TAC[location_name]['CF']['personnel']
            
        # Calculate CF comparing CF_studycase and CF_refcase and total cash flow calculation
        results_TAC[location_name]['CF']['green_hydrogen_incentives'] += results_TAC[location_name]['CF_studycase']['green_hydrogen_incentives']                                                                                                                                                                                                          
        results_TAC[location_name]['CF']['Initial/Final H2 Tank level'] += results_TAC[location_name]['CF_studycase']['Initial/Final H2 Tank level'] 
        results_TAC[location_name]['CF']['Initial/Final NH3 Tank level'] += results_TAC[location_name]['CF_studycase']['Initial/Final NH3 Tank level']
        results_TAC[location_name]['CF']['OeM'] += results_TAC[location_name]['CF_studycase']['OeM']
        results_TAC[location_name]['CF']['Refund'] += results_TAC[location_name]['CF_studycase']['Refund'] 
        results_TAC[location_name]['CF']['CSC'] += results_TAC[location_name]['CF_studycase']['CSC'] 
        
        results_TAC[location_name]['CF_studycase']['Tot'] += results_TAC[location_name]['CF_studycase']['OeM']
        results_TAC[location_name]['CF_studycase']['Tot'] += results_TAC[location_name]['CF_studycase']['Refund']
        results_TAC[location_name]['CF_studycase']['Tot'] += results_TAC[location_name]['CF_studycase']['CSC']

        for carrier in results_TAC[location_name]['CF_studycase']['Purchase']:
            results_TAC[location_name]['CF_studycase']['Tot'] += results_TAC[location_name]['CF_studycase']['Purchase'][carrier]
            results_TAC[location_name]['CF']['Purchase'][carrier] += results_TAC[location_name]['CF_studycase']['Purchase'][carrier]
        
        for carrier in results_TAC[location_name]['CF_studycase']['Sale']:
            results_TAC[location_name]['CF_studycase']['Tot'] += results_TAC[location_name]['CF_studycase']['Sale'][carrier]
            results_TAC[location_name]['CF']['Sale'][carrier] += results_TAC[location_name]['CF_studycase']['Sale'][carrier]

        results_TAC[location_name]['CF']['Tot'] = results_TAC[location_name]['CF_studycase']['Tot'] 
        
        #MC calculate I0 refcase
        results_TAC[location_name]['I0_refcase']['Tot'] = 0
        for tech_name in results_TAC[location_name]['I0_refcase']:
            if tech_name != 'Tot':
                results_TAC[location_name]['I0_refcase']['Tot'] += results_TAC[location_name]['I0_refcase'][tech_name]
                
        #MC calculate I0 studycase
        results_TAC[location_name]['I0_studycase']['Tot'] = 0
        for tech_name in results_TAC[location_name]['I0_studycase']:
             if tech_name != 'Tot':
                 results_TAC[location_name]['I0_studycase']['Tot'] += results_TAC[location_name]['I0_studycase'][tech_name]
        
        #MC calculate IO
        for tech_name in tc[location_name]:
            if tech_name in tc0[location_name]:
                if results_TAC[location_name]['I0_studycase'][tech_name] >= results_TAC[location_name]['I0_refcase'][tech_name]:
                    results_TAC[location_name]['I0'][tech_name] = results_TAC[location_name]['I0_studycase'][tech_name] - results_TAC[location_name]['I0_refcase'][tech_name]
                else:
                    results_TAC[location_name]['I0'][tech_name] = results_TAC[location_name]['I0_studycase'][tech_name]
            else:
                results_TAC[location_name]['I0'][tech_name] = results_TAC[location_name]['I0_studycase'][tech_name]
            results_TAC[location_name]['AF'][tech_name] = (economic_data['interest rate'] * (1 + economic_data['interest rate'])**tc[location_name][tech_name]['replacement']['years']) / ((1 + economic_data['interest rate'])**tc[location_name][tech_name]['replacement']['years'] - 1) 


        results_TAC[location_name]['I0']['Tot'] = 0
        for tech_name in results_TAC[location_name]['I0']:
            if tech_name != 'Tot':
                results_TAC[location_name]['I0']['Tot'] += results_TAC[location_name]['I0'][tech_name]
         
        # Must additional costs for installation, engineering and civil works be taken into account?
        if economic_data['Construction_costs']['application']:
            if economic_data['Construction_costs']['excluded tech']: # for this technologies the installation and engineering costs are already included in the price 
               I = results_TAC[location_name]['I0']['Tot']    
               for tech in economic_data['Construction_costs']['excluded tech']:
                    I = I - results_TAC[location_name]['I0'][tech]
               results_TAC[location_name]['I0']['engineering'] = economic_data['Construction_costs']['engineering']*I
               results_TAC[location_name]['I0']['installation'] = economic_data['Construction_costs']['installation']*I
            else:
                results_TAC[location_name]['I0']['engineering'] = economic_data['Construction_costs']['engineering']*(results_TAC[location_name]['I0']['Tot'])
                results_TAC[location_name]['I0']['installation'] = economic_data['Construction_costs']['installation']*(results_TAC[location_name]['I0']['Tot'])
            results_TAC[location_name]['I0_studycase']['Tot'] = results_TAC[location_name]['I0_studycase']['Tot'] + results_TAC[location_name]['I0_studycase']['installation'] + results_TAC[location_name]['I0_studycase']['engineering'] 

            results_TAC[location_name]['I0']['engineering'] = results_TAC[location_name]['I0_studycase']['engineering']
            results_TAC[location_name]['I0']['installation'] = results_TAC[location_name]['I0_studycase']['installation']
            results_TAC[location_name]['I0']['Tot'] += results_TAC[location_name]['I0']['installation'] + results_TAC[location_name]['I0']['engineering'] 
            results_TAC[location_name]['AF']['engineering'] = (economic_data['interest rate'] * (1 + economic_data['interest rate'])**economic_data['investment years']) / ((1 + economic_data['interest rate'])**economic_data['investment years'] - 1) 
            results_TAC[location_name]['AF']['installation'] = (economic_data['interest rate'] * (1 + economic_data['interest rate'])**economic_data['investment years']) / ((1 + economic_data['interest rate'])**economic_data['investment years'] - 1) 
            
        # calculate TAC
        results_TAC[location_name]['TAC_contributions'] = {}
        for tech_name in results_TAC[location_name]['I0']:
            if tech_name != 'Tot':
                results_TAC[location_name]['TAC_contributions'][tech_name] = results_TAC[location_name]['I0'][tech_name]*results_TAC[location_name]['AF'][tech_name]
        for cash_flow in results_TAC[location_name]['CF']:
            if cash_flow != 'Tot':
                if cash_flow == 'Purchase' or cash_flow == 'Sale':
                    for tech_name in results_TAC[location_name]['CF'][cash_flow]:
                        results_TAC[location_name]['TAC_contributions'][f'{cash_flow} {tech_name}'] = abs(results_TAC[location_name]['CF'][cash_flow][tech_name][0])
                else:
                    results_TAC[location_name]['TAC_contributions'][cash_flow] = abs(results_TAC[location_name]['CF'][cash_flow][0])
        
        results_TAC[location_name]['TAC'] = sum(results_TAC[location_name]['TAC_contributions'].values())
        results_TAC[location_name]['TAC_contributions']['Tot'] = results_TAC[location_name]['TAC']
